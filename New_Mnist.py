#The New_Mnist generation which is based on the Mnist dataset.
#This Dataset contains the same nums at diff. random locations.
#The num. generation is randomlized.
import cPickle
import gzip
import os
import sys
...
def Data_generation(data, index):
    ...
    #define the diff. locations, here two locations, the (x,y) coordination  
    l1_x = np.random.randint(0,72)
    l1_y = np.random.randint(0,72)
    
    l2_x = np.random.randint(0,72)
    l2_y = np.random.randint(0,72)
    #checking the locations random should not be the same
    while (abs(l1_x-l2_x)<28 and abs(l1_x-l2_x)<28):
    ...
    board[l2_x:l2_x+28,l2_y:l2_y+28] = data
...
np.save("New_Mnist_data.npy", New_Mnist_x)
np.save("New_Mnist_label.npy", New_Mnist_y)
